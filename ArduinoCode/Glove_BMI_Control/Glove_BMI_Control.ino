/* Glove Test code for a DUE to test the message handler (BBS)
...
int maxf = 8;       //maximum threshold value to stop lin act (in KG) DO NOT EXCEED 15!!
...
//==================================== BMI Mode ============================================
      ...
      if (target == 0){                     //flex,close hand
        ...
        digitalWrite(trig1,LOW);            //BNC Trigger
        digitalWrite(trig2,HIGH);           //BNC Trigger
        Serial.print("\tPulse Close");      //BNC Trigger
        delay(10);
        digitalWrite(trig1,LOW);            //BNC Trigger
        digitalWrite(trig2,LOW);            //BNC Trigger
          while(forceavg > -maxf && posla>minp){          //Safety limits
            forceavg = constrain((LCL.get_units()-LCR.get_units())/20,-maxf,maxf);   //avg force (-)comp,(+)ext
            pos1 = analogRead(linpot1);                             //position of lin pot 1
            pos2 = 2*analogRead(linpot2)-analogRead(linpot1);       //position of lin pot 2
            pos3 = 2*analogRead(linpot3)-analogRead(linpot4);       //position of lin pot 3
...
//update wire *******************************************!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            Serial.print("\tClosing");}     //Display text
        ...
        flagReady = 0;                  //not ready for command
        digitalWrite(trig1,HIGH);       //BNC trigger
        digitalWrite(trig2,LOW);        //BNC trigger
        Serial.print("\tPulse Open");   //BNC trigger
        delay(10);
        digitalWrite(trig1,LOW);        //BNC trigger
        digitalWrite(trig2,LOW);        //BNC trigger
          while (forceavg < maxf && posla<maxp){                    //open,extend hand
            forceavg = constrain((LCL.get_units()-LCR.get_units())/20,-maxf,maxf);   //avg force (-)comp,(+)ext
            pos1 = analogRead(linpot1);                             //position of lin pot 1
            pos2 = 2*analogRead(linpot2)-analogRead(linpot1);       //position of lin pot 2
            pos3 = 2*analogRead(linpot3)-analogRead(linpot4);       //position of lin pot 3
...
//update wire *******************************************!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            Serial.print("\tOpening");}     //display text
        analogWrite(pwmb,0);            //stop motor
        digitalWrite(incla,LOW);        //stop motor
        digitalWrite(decla,LOW);        //stop motor
        digitalWrite(ledR,LOW);         //red off
...
//=========================================== No Command =================================
  ...
  Serial.print(target);
  Serial.print("\tPos: ");
  Serial.print(sentPos);
  Serial.print("\tForce: ");
  Serial.print(sentForce);
}
