...
while command.planarReady == 0
    [command,posX,posY]         = UpdateArduino(Params.Arduino,Params.Arduino.command,0,0);
    Params.Arduino.command      = command;
    [newX, newY, textHeight]=Screen('DrawText', Params.WPTR, 'Initialising System to Home Position',...
                    50, 50, [255,0,0], [0,0,0]);
...
if ~Data.ErrorID && ~Params.CenterReset,
    ...
    InTargetTotalTime = 0;
    while ~done,
        Screen('DrawText', Params.WPTR, 'Goto start target',50, 50, [255,0,0], [0,0,0]);
        % Update Time & Position
        tim = GetSecs;
...
Params.Arduino.command.target           = 0;    % 1-bit     Move to target, or accept sent velocities
[command,posX,posY]                     = UpdateArduino(Params.Arduino,Params.Arduino.command,0,0);
Params.Arduino.command                  = command;
pause(0.1)
while command.planarReady == 0
    [command,posX,posY]         = UpdateArduino(Params.Arduino,Params.Arduino.command,0,0);
    Params.Arduino.command      = command;
    [newX, newY, textHeight]=Screen('DrawText', Params.WPTR, 'Auto-correcting postion',...
                    50, 50, [255,0,0], [0,0,0]);
    Screen('Flip', Params.WPTR);
    pause(0.1)
end
...
if ~Data.ErrorID && Params.InstructedDelayTime>0,
    ...
    InTargetTotalTime = 0;
    while ~done,        
        Screen('DrawText', Params.WPTR, 'Instruction Delay',50, 50, [255,0,0], [0,0,0]);
        % Update Time & Position
        tim = GetSecs;
...
if ~Data.ErrorID,
    ...
    InTargetTotalTime = 0;
    while ~done,
        Screen('DrawText', Params.WPTR, 'Go-to reach target',50, 50, [255,0,0], [0,0,0]);
        % Update Time & Position
        tim = GetSecs;
